...
module.exports = function(req, res, action, next){
	async.waterfall([
		function(callback){
			files.addFile(action, req.session.user, "all", callback);
		},
		function(tmpFile, callback){
			var userId = req.session.user;
			var date = Date.now().toString();
			var data = [tmpFile._id, date, userId, action];
			data = data.join("/");
			...
			var reqToStServer = domain.create();
			reqToStServer.run(function(){
			...
			reqToStServer.on('error', function(err){
				log.error(err);
				next(500);
			})
